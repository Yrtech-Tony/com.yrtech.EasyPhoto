...
namespace XHX.Common
{
    public class MSExcelUtil
    {
        ...
        public void SetCellFont(Worksheet worksheet,  string x, int y, object value)
        {
            int xInt = ColumnToIndex(x);
            (worksheet.Cells[y, xInt] as Range).Font.Name = value.ToString();
            //(worksheet.Cells[y, xInt] as Range).Font.Name = "Arial";
        ...
        public void InsertPicture(Worksheet m_objSheet, Range RangeName, string PicturePath, int rowIndex)
        {
            ...
            PicLeft = Convert.ToSingle(m_objRange.Left) + 1.5f;
           // PicTop = Convert.ToSingle(m_objRange.Top) - ((float)rowIndex - 2f) * 0.3f;
